...
function mapXB(code = '1') {
  const dict = { 1: '男', 2: '女' };
  return dict[code] || code;
}
function mapPYFS(code = '1') {
  const dict = { 0: '----', 1: '非定向', 2: '定向', 9: '其他' };
  return dict[code] || code;
}
const dict = {};
...
async function doWork() {
  ...
  for (let i = 0; i < rs.length; i++) {
    ...
    // TODO: 字典转换 xb, mz, syszd, xl, zzmm
    try {
      data.year = data.bysj.substr(0, 4);
      data.type = mapType(data.xldm);
      data.meta = meta;
      data.xb = mapXB(data.xbdm);
      data.pyfs = mapPYFS(data.pyfsdm);
